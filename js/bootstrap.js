/*!
 ...
 (function (global, factory) {
  ...
  function bootstrapDelegationHandler(element, selector, fn) {
    ...
    // 委托事件
    return function handler(event) {
      var domElements = element.querySelectorAll(selector); // 选择器

      for (var target = event.target; target && target !== this; target = target.parentNode) { // 事件冒泡
        for (var i = domElements.length; i--;) {
          if (domElements[i] === target) { // 与选择器匹配调用处理器
  ...
  /**
   * 事件处理对象
   * bootstrap的事件处理对象会在一开始为所有插件的dom添加上事件
   * 添加事件的方式是在 document上添加，在事件处理器中判断，匹配各自的dom元素
   */
  var EventHandler = {
  ...
  function () {
    function Alert(element) {
      ...
      if (this._element) {
        console.log('set')
        Data.setData(element, DATA_KEY, this); // 储存dom元素
      }
    } // Getters
    ...
    _proto.close = function close(element) {
      ...
      if (element) { // 当前 alert dom元素，通过apply 继承传过来
        rootElement = this._getRootElement(element);
      }
      console.log('element2',rootElement)
    ...
    Alert._handleDismiss = function _handleDismiss(alertInstance) {
      return function (event) { // event 是apply 传过来的
        console.log(123,eventRegistry);
        mapData.all();
        if (event) {
  ...
  // console.log(Event$1.CLICK_DATA_API, Selector.DISMISS) 
